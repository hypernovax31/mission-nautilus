...
MOTS_CANDIDATS = [
    dict(reponse='DEMATERIALISATION', secteur='rayon',     mode=None,
         indice="la retraite du support : ce qui tournait encore hier dans un boîtier de plastique s'efface pour ne plus vivre qu'à distance, dans un entrepôt de données — on achète le droit d'accéder, plus jamais l'objet lui-même"),
    dict(reponse='TRANSFORMATION',    secteur='direction', mode=None,
         indice="l'aiguille du verdict : parmi ceux qui franchissent la barrière lumineuse du seuil, combien ressortiront réellement les bras chargés ? Cette proportion-là, et nulle autre, dit si la boutique convertit sa foule ou la laisse filer"),
    dict(reponse='STEELBOOK',         secteur='rayon',     mode=None,
         indice="la cuirasse de collection : l'écrin de métal, nettement plus pesant qu'une jaquette de plastique, qui enveloppe le film d'une seconde peau et justifie aux yeux du chineur un prix que la seule galette ne porterait jamais"),
    dict(reponse='DIAGNOSTIC',        secteur='sav',       mode=None,
         indice="la consultation du comptoir technique : recueillir les symptômes, palper la machine, interroger ses entrailles, puis rendre l'arrêt — panne réelle ou simple maladresse de l'utilisateur ? Le verdict décide de l'atelier ou du retour au client"),
    dict(reponse='REASSORT',          secteur='stock',     mode=None,
         indice="l'office de l'aurore : la ronde furtive entre l'arrière-salle et le front de vente, pour que chaque trou laissé la veille soit comblé avant que la porte ne s'ouvre — broche après broche, étagère après étagère"),
    dict(reponse='PICKING',           secteur='stock',     mode=None,
         indice="la récolte de l'invisible : arpenter les travées du dépôt, liste en main, pour rassembler pièce à pièce la commande d'un inconnu qui paiera sans jamais croiser le chemin de celui qui la compose"),
    dict(reponse='INVENTAIRE',        secteur='bureau',    mode=None,
         indice="la nuit où les comptes descendent sur terre : la douchette s'allume, chaque référence défile, et le chiffre affiché à l'écran doit enfin se mesurer à la vérité des rayonnages — le virtuel rend gorge au réel"),
    dict(reponse='PLEIADE',          secteur='rayon',     mode=None,
         indice="l'aristocratie de l'imprimé : la collection au dos souple, bordé d'un liseré d'or, qu'on ne solde jamais et qu'on glisse dans le papier cadeau des grandes occasions — l'édition que l'on garde une vie entière"),
    dict(reponse='PLANOGRAMME',       secteur='direction', mode=None,
         indice="le cadastre des rayons : la carte immuable descendue du siège, qui fige au millimètre la place de chaque référence et fait loi sur la boutique — un produit hors de sa case est un produit en faute"),
    dict(reponse='FACING',            secteur='rayon',     mode=None,
         indice="la ligne de front : ramener chaque article jusqu'au rebord même de l'étagère, sans un vide, sans une bavure, jusqu'à ce que le rayon tout entier présente une façade lisse — la discipline du neuf perpétuel"),
]
